...
function tribe_load_image_fb_widget() {
	register_widget('Tribe_Image_Fb_Widget');
}
add_action('widgets_init', 'tribe_load_image_fb_widget');

/**
 * Tribe_Image_Fb_Widget class
 **/
class Tribe_Image_Fb_Widget extends WP_Widget {
	 ...
	 * @author Modern Tribe, Inc.
	 */
	function Tribe_Image_Fb_Widget() {
      ...
      //	$widget_ops = array( 'classname' => 'widget_image', 'description' => __( 'Showcase a single image with a Title, URL, and a Description', 'image_widget' ) );
        $widget_ops = array( 'classname' => 'widget_fb_image', 'description' => 'Facebookのリンクです' );

      $control_ops = array( 'id_base' => 'widget_fb_image' );
		$this->WP_Widget('widget_fb_image', 'facebook', $widget_ops, $control_ops);
	...
	private static function get_defaults() {
		...
		$defaults = array(
			'title' => '',
			'description' => '',
          'link' => 'https://www.facebook.com/realglobe',
			'linktarget' => '',
			'width' => 0,
			'height' => 0,
			'maxwidth' => '100%',
			...
			'imageurl' => $url . '/wp-content/uploads/2013/10/btn_facebook.png', // reverse compatible.
			'align' => 'none',
			'alt' => '',
		);
