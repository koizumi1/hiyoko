...
function tribe_load_image_tw_widget() {
	register_widget('Tribe_Image_Tw_Widget');
}
add_action('widgets_init', 'tribe_load_image_tw_widget');

/**
 * Tribe_Image_Tw_Widget class
 **/
class Tribe_Image_Tw_Widget extends WP_Widget {
	 ...
	 * @author Modern Tribe, Inc.
	 */
	function Tribe_Image_Tw_Widget() {
		load_plugin_textdomain( 'image_widget', false, trailingslashit(basename(dirname(__FILE__))) . 'lang/');
      //	$widget_ops = array( 'classname' => 'widget_image', 'description' => __( 'Showcase a single image with a Title, URL, and a Description', 'image_widget' ) );
        $widget_ops = array( 'classname' => 'widget_tw_image', 'description' => 'Twitterのリンクです' );

      $control_ops = array( 'id_base' => 'widget_tw_image' );
		$this->WP_Widget('widget_tw_image', 'twitter', $widget_ops, $control_ops);
	...
	private static function get_defaults() {
		...
		$defaults = array(
			'title' => '',
			'description' => '',
			'link' => 'https://twitter.com/realglobeInc',
			'linktarget' => '',
			'width' => 0,
			'height' => 0,
			'maxwidth' => '100%',
			...
			'imageurl' => get_bloginfo('url') . "/wp-content/uploads/2013/10/btn_twitter1.png", // reverse compatible.
			'align' => 'none',
			'alt' => '',
		);
